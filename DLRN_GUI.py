...
class MyWindow(QMainWindow):
    ...
    def on_combobox1_changed(self, index):
        ...
        if selected_option == "Spectra":
            ...
            try:
                path_model = path + "/pretrained_model/Spectra/resV2_spectra_model"
                model_model = tf.keras.models.load_model(path_model, custom_objects={'top_3_acc':top_3_acc})
                self.model_model = model_model

                path_amp = path + "/pretrained_model/Spectra/resV2_spectra_amplitude" # 
                model_amp = tf.keras.models.load_model(path_amp)
                self.model_amp = model_amp

                path_tau = path + "/pretrained_model/Spectra/resV2_spectra_Tau"
                model_tau = tf.keras.models.load_model(path_tau)
                self.model_tau = model_tau

            except:
                path_model = path + "/pretrained_model/pretrained_model/Spectra/resV2_spectra_model"
                model_model = tf.keras.models.load_model(path_model, custom_objects={'top_3_acc':top_3_acc})
                self.model_model = model_model
    ...
    def load_time_scale(self):
        ...
        if self.adjusting_factor == -1:
            ...
            self.adjusting_factor = 1000/np.max(time_original)

        else:
            self.adjusting_factor = 1/self.adjusting_factor

        self.adjusting_factor = self.adjusting_factor
        self.time_original = time_original*self.adjusting_factor
    ...
    def Analyse_data(self):
        ...
        for i in range (len(top_3_indices)):
            ...
            Tau_solutions[i,:] = pre_tau[0,:]/self.adjusting_factor
